...
## directories: create directories to save feature, models, results 
input_dir = paste0(root,"/00_input") # raw data directory
feat_dir = paste0(root,"/01_feature") # feature directory
...
keep = subset(data_expl, variance>quantile(data_expl$variance, 0.3) & corr_p>quantile(abs(data_expl$corr_p), 0.3))

data2 = subset(data0, select=keep$col)

## save for future use
write.csv(data2, paste0(feat_dir,'/feature_trraw.csv'), row.names=T)
...
#write.csv(feat.unique, paste0(feat_dir,'/feature_tra.csv'), row.names=T)
```
